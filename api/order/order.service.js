...
async function query(filterBy = {}) {
    ...
    try {
        ...
        let orders = await collection.find(criteria).sort({ _id: -1 }).toArray()
...
async function add(order) {
    ...
    try {
        const orderToAdd = {
            ...
            byUser: { fullname: order.byUser.fullname, id: ObjectId(order.byUser.id), email: order.byUser.email },
            startDate: order.startDate,
            endDate: order.endDate,
            guests: order.guests,
            status: order.status,
            byStayId: ObjectId(order.byStayId)
        }
        const collection = await dbService.getCollection('order')
